...
func TestLFU_Set(t *testing.T) {
	...
	if evicted := s.Set("key", "value"); evicted {
		t.Errorf("expect false got %v", evicted)
	}
}
...
func TestLFU_GetOrSet(t *testing.T) {
	s := NewLFUCache(32)
...
func TestLFU_PopOldest(t *testing.T) {
	s := NewLFUCache(32)
	s.Set(1, 2)
...
func BenchmarkLFU_GetExist(b *testing.B) {
	b.StopTimer()
	s := NewLFUCache(8096)
	s.Set("key", "value")
	b.StartTimer()
...
func BenchmarkLFU_GetNotExist(b *testing.B) {
	b.StopTimer()
	s := NewLFUCache(8096)
	b.StartTimer()
	for i := 0; i < b.N; i++ {
